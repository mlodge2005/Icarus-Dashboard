...
export const create = mutation({ args: { title: v.string(), url: v.optional(v.string()), note: v.optional(v.string()), now: v.string() }, handler: async (ctx, a) => ctx.db.insert("documents", { title: a.title, url: a.url, note: a.note, createdAt: a.now, updatedAt: a.now }) });
